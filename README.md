...
| `ARCHITECTURE.md` | Cross-service architecture and data flow for the OVP + ttrpg-collector stack. |
...
- `ovp-data-api` — Rust, storage abstraction (Postgres + S3)
- `ovp-pipeline` — Rust, transcription library (VAD + Whisper + operators)
- `ovp-worker` — Rust, pipeline orchestrator
- `ttrpg-collector` — Rust, Discord bot (voice capture with consent)
- `ttrpg-collector-frontend` — Next.js/TS participant portal
